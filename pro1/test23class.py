...
# 2) 기울기는 선형화귀를 거쳐 미분•Gradient Descent, 딥러닝의 학습 원리로 연결된다.
...
class CalcTest:
    def __init__(self, x1, y1, x2, y2, offset:float=1.0): # 두 점의 좌표 얻기
        ...
        # 로그는 0과 음수를 허용하지 않으므로 offset로 여기에 대체한다.

        self.offset = offset

    # 두 점 사이의 거리(유클리그 거리 계산식) a² + b² = c²
    ...
    def transform(self, x_list:list[float]):  # 로그 변환
        return [math.log(x + self.offset) for x in x_list] # 클래스는 클래스를 부름 포홤 관계와 상속 관계가 있다. 자원의 재활용
...
def main():
    ...
    reversed_data_round = [round(val, 1) for val in reversed_data] #round : 반올림
