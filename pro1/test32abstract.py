...
# 추상 클래스 = 자식 클래스에게 규칙을 강제(메소드 오버라이딩)하는 부모 클래스

from abc import *

class AbstractClass(metaclass = ABCMeta): # 추상 클래스, 객체로 만들수가 없다.

    @abstractmethod  # abc메소드를 추상메소드로 선언
...
# c1 = Child1() # 추상메소드 오버라이딩 X - TypeError: Can't instantiate abstract class
ch1 = Child1()
print('name : ', ch1.name)
ch1.abcMethod()
ch1.normalMethod()
