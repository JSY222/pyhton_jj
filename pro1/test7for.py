...
print(datas.items())  # [('만능언어','웹용언어'), ... ] 결과를 반환
...
print('\n\n정규표현식 + for 연습 ---')
message = """
도널드 트럼프 미국 대통령이 이재명 대통령과 최근통화에서 이란전쟁에 대한 한국의
17일(현지시간) 트럼프 대통령은 백악관에서 취재진과 만나 최근 이 대통령과 통화한 
도널드 트럼프 미국 대통령이 이재명 대통령과 최근통화에서
도널드 트럼프 도널드 트럼프 도널트트럼프 abc & * $ # @
"""
import re
message2 = re.sub(r'[^가-힣\s]', '', message) # 패턴과 일치하는 문자열을 다른 문자열로 치환
print(message2)
message3 = message2.split(' ') # 공백 기준 문자열 분리, split 자르다
print(message3)
# 단어별 빈도수 출력 : dict 사용
cou = {}
for i in message3: # 변수는 임의로 설정하나 주로 i 사용
    if i in cou:
...
name_id = { val:key for key, val in id_name.items()} # for key 수행하고 val:key가 결과값
