...
class PohamCar:
    ...
    def __init__(self, ownerName): # 전부 톰이다.
    ...
    def turnHandle(self, q):  
        ...
        if q > 0:
            ...
            self.turnShowMessage = self.handle.lefTurn(q)
        elif q == 0:
...
# PomhamCar는 객체 owername 가지고있고  handle = PohamHandale이고 메소드는 turnHandle() 
# pomhamHandle은 객체 quantity 가지고있고 메소드는 lefttrun(), rightturn()
# owername = '미스터 톰', 객체가 4개만들어짐
    print ()
    suji = PohamCar("미스 수지")
    suji.turnHandle(-20)
